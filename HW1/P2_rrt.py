    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            ...
            if z<goal_bias:
                ...
                x_rand = np.random.uniform(self.statespace_lo,state_dim)
            ...
            if self.is_free_motion(self.obstacles,x_near,x_new):
                ...
                if np.linalg.norm(x_new-self.x_goal)<0.01:
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        return (eps * (x2 - x1) / np.linalg.norm(x2 - x1) + x1) if np.linalg.norm(x2 - x1) > eps else x2
        ########## Code ends here ##########
        pass
